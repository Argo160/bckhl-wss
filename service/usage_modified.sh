    ...
    for log_file in "$log_dir"/*.json; do
        if [[ -f $log_file ]]; then
            ...
            if [[ "${service_status[$service_name]}" != "$current_status" ]]; then
                ...
                # Call send_status.py if the service is disconnected and button not already sent
                if [[ "$current_status" == "DISCONNECTED ❌" && -z "${button_sent[$service_name]}" ]]; then
                    python3 /root/send_status_modified.py "$service_name" "$server_name"  # Send the message with the button
                    echo "$service_name"
...
# Start checking
python3 /root/button_listener_modified.py "$server_name" &
check
